                ...
                <div className="card">
                    <div className="card-front">
                        <img
                            ...
                            alt="NFT"
                        />
                        <h2 className={"h2"}>{data.item.name} #{data.item.item_id || data.item.seed_id}</h2>
                        <p className="description">{data.item.type === 'seed' ? 'Семена' : 'Предмет проклятой коллекции'}</p>
                        <div className="tokenInfo">
                            <div className="price">
                                <ins>◘</ins>
                                <p>{data.item.price || 'N/A'} DOGS</p>
                            </div>
                            <div className="duration">
                                <ins>◷</ins>
                                <p>{data.item.type === 'seed' ? `${data.item.growth_time} Growth Time` : data.item.image}</p>
                            </div>
                        </div>
                        ...
                        <div className="back-content">
                            ...
                            {data.item.type === 'seed' && (
                                <>
                                    <div className="item-info">
                                        <span>Редкость:</span>
                                        <span className="item-value">{data.item.rarity}</span>
                                    </div>
                                    <div className="item-info">
                                        <span>Стадия:</span>
                                        <span className="item-value">{data.item.stage}</span>
                                    </div>
                                    <div className="item-info">
                                        <span>Время роста:</span>
                                        <span className="item-value">{data.item.growth_time}</span>
                                    </div>
                                </>
                            )}
                        </div>
